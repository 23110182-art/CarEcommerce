...
class CarService {
  async _applyPromotionsToCarsResult(result) {
    ...
    result.cars = result.cars.map((car) => {
      ...
      const discount = promotionService.calculateDiscountForCar(
        carObj,
        activePromotions,
      );
      if (discount) {
  ...
  async deleteCar(id) {
    ...
    if (car.thumbnail) {
      await deleteFromCloudinary(car.thumbnail);
    }

    if (car.images && car.images.length > 0) {
      await Promise.all(
        car.images.map((image) => deleteFromCloudinary(image.url || image)),
      );
    }
  ...
  async getFeaturedCars() {
    const result = await carRepository.findAllWithFilters({
      isFeatured: true,
      limit: 8,
    });
    return await this._applyPromotionsToCarsResult(result);
