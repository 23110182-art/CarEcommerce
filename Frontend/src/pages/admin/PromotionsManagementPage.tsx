...
import { Table, Button, Space, Modal, Form, Input, Select, InputNumber, DatePicker, Switch, message, Typography, Popconfirm, Tag, Row, Col } from 'antd';
...
const PromotionsManagementPage: React.FC = () => {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => {
      message.error(err.response?.data?.message || 'Failed to create promotion');
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => promotionApi.updatePromotion(id, data),
    onSuccess: () => {
    ...
    onError: (err: any) => {
      message.error(err.response?.data?.message || 'Failed to update promotion');
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => {
      message.error(err.response?.data?.message || 'Failed to delete promotion');
    },
  });
  ...
  const handleSubmit = (values: any) => {
    const formattedValues = {
      ...
      applicable_brands: values.apply_to === 'brand' ? values.applicable_brands : [],
      applicable_categories: values.apply_to === 'category' ? values.applicable_categories : [],
      applicable_cars: values.apply_to === 'specific_cars' ? values.applicable_cars : [],
    };
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any) => (
        <Space direction="vertical" size={2}>
          <Text style={{ color: 'white', fontWeight: 600, fontSize: '15px' }}>{text}</Text>
          <Paragraph ellipsis={{ rows: 1 }} style={{ color: 'var(--color-text-secondary)', margin: 0, fontSize: '12px', maxWidth: '250px' }}>
            {record.description}
          </Paragraph>
        </Space>
      ),
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        return <Tag color={color} style={{ textTransform: 'uppercase', fontWeight: 600 }}>{label}</Tag>;
      },
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        return <Text style={{ color: 'var(--color-text-secondary)' }}>{start} - {end}</Text>;
      },
    },
    {
      title: 'Status',
      ...
      render: (active: boolean) => (
        <Tag color={active ? 'success' : 'default'} style={{ fontSize: '12px', textTransform: 'uppercase', fontWeight: 'bold' }}>
  ...
  return (
    <div style={{ padding: '30px', background: 'transparent' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <div>
          <Title level={2} style={{ color: 'white', margin: 0 }}>Promotions Management</Title>
          <Text style={{ color: 'var(--color-text-secondary)' }}>Manage discount campaigns and pricing rules</Text>
        </div>
        <Button
          type="primary"
          icon={<Plus size={16} />}
          onClick={() => handleOpen(null)}
          style={{ height: '40px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}
        >
          Add Campaign
        </Button>
      </div>
      ...
      <Modal
        title={<Title level={3} style={{ margin: 0, color: 'white', fontFamily: 'Outfit, sans-serif' }}>{editingId ? 'Edit Promotion Campaign' : 'Create Promotion Campaign'}</Title>}
        open={isModalOpen}
        ...
        >
          <Row gutter={16}>
            ...
            <Col xs={24} sm={8}>
              ...
              >
                <Switch checkedChildren="Active" unCheckedChildren="Inactive" style={{ marginTop: '6px' }} />
              </Form.Item>
            </Col>
            ...
            <Col xs={24}>
              ...
              >
                <Input.TextArea placeholder="Provide detail regarding this discount campaign" rows={3} />
              </Form.Item>
            </Col>
            ...
            <Col xs={24} sm={12}>
              <Form.Item
                name="discount_type"
                label="Discount Type"
                rules={[{ required: true, message: 'Please choose discount type' }]}
              >
                <Select size="large">
            ...
            <Col xs={24} sm={12}>
              ...
              >
                <InputNumber
                  ...
                  size="large"
                  min={0}
                  placeholder={applyToValue === 'percentage' ? 'e.g. 10' : 'e.g. 50000'}
                />
              </Form.Item>
            </Col>
            ...
            <Col xs={24}>
              <Form.Item
                ...
                rules={[{ required: true, message: 'Please choose target scope' }]}
              >
                <Select size="large">
                  <Option value="all">Entire Showroom (All Cars)</Option>
                  <Option value="brand">Applicable Hãng xe (Brands)</Option>
                  <Option value="category">Applicable Dòng xe (Categories)</Option>
                  <Option value="specific_cars">Specific Vehicles (Chọn từng xe)</Option>
                </Select>
              </Form.Item>
            </Col>
            ...
            {applyToValue === 'brand' && (
              <Col xs={24}>
                ...
                >
                  <Select mode="multiple" placeholder="Select applicable brands" size="large" style={{ width: '100%' }}>
                    {brands?.map(b => (
            ...
            {applyToValue === 'category' && (
              <Col xs={24}>
                ...
                >
                  <Select mode="multiple" placeholder="Select applicable categories" size="large" style={{ width: '100%' }}>
                    {categories?.map(c => (
            ...
            {applyToValue === 'specific_cars' && (
              <Col xs={24}>
                <Form.Item
                  ...
                  rules={[{ required: true, message: 'Please select at least one Car' }]}
                >
          ...
          <Space style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '24px' }}>
